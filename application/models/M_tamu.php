...
class M_tamu extends CI_Model{
	...
	function get_all_tamu(){
		return $this->db->count_all("tbl_tamu");
	}

	function get_tamu() {
		$this->db->select('tbl_tamu.*,tbl_pegawai.nama as namatujuan, tbl_kartu_tamu.serial_kartu as serial_kartu');
		$this->db->from('tbl_tamu');
		$this->db->join('tbl_pegawai','tbl_pegawai.id = tbl_tamu.nama_tujuan','LEFT');
		$this->db->join('tbl_kartu_tamu', 'tbl_kartu_tamu.id_tamu = tbl_tamu.id', 'left');
		return $this->db->get();
	}

	public function filterTamuByTanggal($dari_tgl, $ke_tgl) {
		$this->db->select('tbl_tamu.*, tbl_pegawai.nama as namatujuan, tbl_kartu_tamu.serial_kartu as serial_kartu, tbl_kartu_tamu.`status` as status_kartu, tbl_lampiran.`file` as lampiran');
        $this->db->from('tbl_tamu');
        $this->db->join('tbl_pegawai', 'tbl_pegawai.id = tbl_tamu.nama_tujuan', 'left');
        $this->db->join('tbl_kartu_tamu', 'tbl_kartu_tamu.id_tamu = tbl_tamu.id', 'left');
        $this->db->join('tbl_lampiran', 'tbl_lampiran.id_tamu = tbl_tamu.id', 'left');
        $this->db->where('tgl_datang >=', $dari_tgl);
        $this->db->where('tgl_datang <=', $ke_tgl);
